...
d3.custom.pieChart = function module() {
	...
	// D3 helper function to populate pie slice parameters from
	// array data
	...
	// D3 helper function to draw arcs, populates parameter "d"
	// in path object
	...
	function exports(_selection) {
		_selection
				.each(function(_data) {
					var visible = angular.element(this).find("svg").length;
					...
					if (!visible || !vis) {
						...
						vis = d3.select(this).append("svg:svg").attr("xmlns",
								"http://www.w3.org/2000/svg").attr("version",
								"1.1").attr("width", w).attr("height", h).attr(
								"viewBox", "0 0 " + w + " " + h).attr(
								"preserveAspectRatio", "xMinYMin meet");

						// GROUP FOR ARCS/PATHS
						arc_group = vis.append("svg:g").attr("class", "arc")
								.attr(
										"transform",
										"translate(" + (w / 2) + "," + (h / 2)
												+ ")");
						...
						// GROUP FOR LABELS
						label_group = vis.append("svg:g").attr("class",
								"label_group").attr("transform",
								"translate(" + (w / 2) + "," + (h / 2) + ")");

						// GROUP FOR CENTER TEXT
						center_group = vis.append("svg:g").attr("class",
								"center_group").attr("transform",
								"translate(" + (w / 2) + "," + (h / 2) + ")");

						// PLACEHOLDER GRAY CIRCLE
						paths = arc_group.append("svg:circle").attr("fill",
								"#EFEFEF").attr("r", r);
						...
						whiteCircle = center_group.append("svg:circle").attr(
								"fill", "white").attr("r", ir);
					...
					function update() {
						...
						if (filteredPieData.length > 0 && oldPieData.length > 0) {
							...
							// DRAW ARC PATHS
							paths = arc_group.selectAll("path").data(
									filteredPieData);
							paths.enter().append("svg:path").attr("stroke",
									"white").attr("stroke-width", 0.5).attr(
									"fill", function(d, i) {
										return color(i);
							...
							paths.exit().transition().duration(tweenDuration)
									.attrTween("d", removePieTween).remove();

							// DRAW TICK MARK LINES FOR LABELS
							lines = label_group.selectAll("line").data(
									filteredPieData);
							lines
									.enter()
									.append("svg:line")
							...
							lines.transition().duration(tweenDuration).attr(
									"transform",
									function(d) {
										return "rotate("
												+ (d.startAngle + d.endAngle)
							...
							valueLabels.transition().duration(tweenDuration)
